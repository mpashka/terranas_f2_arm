...
#ifdef RTD129X_MESH_DEMO
#define betterPath(xmetric, xhop, ymetric, yhop) (( ((xmetric + pathSwitchThreshold) < ymetric) || ( (xmetric <= ymetric) && (xhop < yhop) ) ) && (xhop < 3) )
#else
	   ...
	   int My_interFace)
{
...
#ifdef RTD129X_MESH_DEMO
    if(qResult > 0 && pathEntry.dsn <= dsn && hopcount < 3) {
#else
    if(qResult > 0 && pathEntry.dsn <= dsn) {
#endif
#if 0
        if(destMAC[4]==0xF3 && destMAC[5]==0xD3) {
            PathselLog(" Current nexthopMAC %02X:%02X:%02X:%02X:%02X:%02X Metric=%d hop=%d\n", pathEntry.nexthopMAC[0],pathEntry.nexthopMAC[1],pathEntry.nexthopMAC[2],pathEntry.nexthopMAC[3],pathEntry.nexthopMAC[4],pathEntry.nexthopMAC[5], pathEntry.metric , pathEntry.hopcount);
            PathselLog("Cadidate nexthopMAC %02X:%02X:%02X:%02X:%02X:%02X Metric=%d hop=%d\n", My_Prehop[0],  My_Prehop[1], My_Prehop[2], My_Prehop[3], My_Prehop[4], My_Prehop[5], metric, hopcount);
        }
	...
	else if( betterPath(metric, hopcount, pathEntry.metric, pathEntry.hopcount)) {
#if 0
        if(destMAC[4]==0xF3 && destMAC[5]==0xD3) {
            PathselLog("Change path() destMac(%02X:%02X:%02X:%02X:%02X:%02X)\n", destMAC[0],destMAC[1],destMAC[2],destMAC[3],destMAC[4],destMAC[5]);
            PathselLog("Original nexthopMAC %02X:%02X:%02X:%02X:%02X:%02X Metric=%d hop=%d\n", pathEntry.nexthopMAC[0],pathEntry.nexthopMAC[1],pathEntry.nexthopMAC[2],pathEntry.nexthopMAC[3],pathEntry.nexthopMAC[4],pathEntry.nexthopMAC[5], pathEntry.metric , pathEntry.hopcount);
	        PathselLog("     New nexthopMAC %02X:%02X:%02X:%02X:%02X:%02X Metric=%d hop=%d\n", My_Prehop[0],  My_Prehop[1], My_Prehop[2], My_Prehop[3], My_Prehop[4], My_Prehop[5], metric, hopcount);
           }
#endif
            memcpy(pathEntry.destMAC, destMAC, MACADDRLEN);
            memcpy(pathEntry.nexthopMAC, My_Prehop, MACADDRLEN);	    
