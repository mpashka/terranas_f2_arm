...
int main(int argc, char *argv[]) {
	...
	data1.videodepay = gst_element_factory_make ("rtph264depay", "videodepay");
#ifdef REC_AUDIO
...
#ifdef REC_AUDIO
	if (!data1.pipeline || !data1.source || !data1.videodepay || !data1.audiodepay || !data1.queue_disp || !data1.queue_rec || !data1.mux_disp || !data1.mux_rec || !data1.tee || !data1.sink_file || !data1.sink_rtk) {
		g_printerr ("Not all elements could be created.\n");
		return -1;
	}
#else
	if (!data1.pipeline || !data1.source || !data1.videodepay || !data1.queue_disp || !data1.queue_rec || !data1.mux_disp || !data1.mux_rec || !data1.tee || !data1.sink_file || !data1.sink_rtk) {
		g_printerr ("Not all elements could be created.\n");
		return -1;
	}
#endif
...
#ifdef REC_AUDIO
	gst_bin_add_many (GST_BIN (data1.pipeline), data1.source, data1.videodepay, data1.audiodepay, data1.tee, data1.queue_disp, data1.queue_rec, data1.mux_rec, data1.sink_file, data1.mux_disp, data1.sink_rtk, NULL);
#else
	gst_bin_add_many (GST_BIN (data1.pipeline), data1.source, data1.videodepay, data1.tee, data1.queue_disp, data1.queue_rec, data1.mux_rec, data1.sink_file, data1.mux_disp, data1.sink_rtk, NULL);
#endif
	if (!gst_element_link (data1.videodepay, data1.tee) ||
#ifdef REC_AUDIO
...
static void pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  if(gst_element_link_pads(data->source, dynamic_pad_name, data->videodepay, "sink")){
    g_print("Pad for video linked");
