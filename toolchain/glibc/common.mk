...
define Host/PatchShebang
	find \
	    $(TOOLCHAIN_DIR)/bin \
	    -type f -executable -exec grep -Iq "$(REAL_STAGING_DIR_HOST)" {} \; -and -print \
	    | xargs --no-run-if-empty sed -i -r 's,^#!.?$(subst +,\+,$(REAL_STAGING_DIR_HOST))(/usr)?/bin/([^ ]+) *.*$$$$,#!/usr/bin/env \2,g'
endef
