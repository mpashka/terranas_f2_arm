...
#define SND_REALTEK_DRIVER_I2S_IN_multicap "snd_alsa_rtk_i2s_in_multicap"
...
static int snd_card_capture_i2s_open_multicap(snd_pcm_substream_t * substream);
...
static int snd_card_capture_close_multicap(snd_pcm_substream_t * substream);
static int snd_card_capture_ioctl(struct snd_pcm_substream *substream,  unsigned int cmd, void *arg);
static int snd_card_analog_capture_ioctl(struct snd_pcm_substream *substream,  unsigned int cmd, void *arg);
static int snd_card_capture_prepare(snd_pcm_substream_t * substream);
static int snd_card_capture_prepare_multicap(snd_pcm_substream_t * substream);
...
static void snd_card_capture_lpcm_timer_function_multicap(unsigned long data);
...
#if 1 // __LINUX_MEDIA_NAS__
static int snd_RTK_switch_hdmi_info(snd_kcontrol_t * kcontrol, snd_ctl_elem_info_t * uinfo);
static int snd_RTK_switch_hdmi_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
static int snd_RTK_switch_hdmi_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
static int snd_RTK_switch_i2s_info(snd_kcontrol_t * kcontrol, snd_ctl_elem_info_t * uinfo);
static int snd_RTK_switch_i2s_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
static int snd_RTK_switch_i2s_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
static int snd_RTK_switch_dac_info(snd_kcontrol_t * kcontrol, snd_ctl_elem_info_t * uinfo);
static int snd_RTK_switch_dac_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
static int snd_RTK_switch_dac_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
static int snd_RTK_switch_spdif_info(snd_kcontrol_t * kcontrol, snd_ctl_elem_info_t * uinfo);
static int snd_RTK_switch_spdif_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
static int snd_RTK_switch_spdif_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol);
#endif
...
static char *snd_pcm_id[] = {SND_REALTEK_DRIVER, SND_REALTEK_DRIVER_I2S_IN, SND_REALTEK_DRIVER_NONPCM_IN,
			SND_REALTEK_DRIVER_ANALOG_IN, SND_REALTEK_DRIVER_I2S_IN_multicap}; // multiple PCM instances in 1 sound card
...
int adc3x01_i2c_amic_setup(void);
...
static struct ion_handle *alsa_capture_handle_multicap = NULL;
static struct ion_handle *enc_in_handle[2] = {NULL};
static struct ion_handle *enc_lpcm_handle = NULL;
static struct ion_handle *enc_lpcm_handle_multicap = NULL;
...
#if 1 // __LINUX_MEDIA_NAS__
MARS_SWITCH_HDMI("HDMI Playback Switch", 0, MIXER_ADDR_MASTER),
MARS_SWITCH_I2S("I2S Playback Switch", 0, MIXER_ADDR_MASTER),
MARS_SWITCH_DAC("DAC Playback Switch", 0, MIXER_ADDR_MASTER),
MARS_SWITCH_SPDIF("SPDIF Playback Switch", 0, MIXER_ADDR_MASTER),
#endif
...
// for I2S in (multicap)
static snd_pcm_ops_t snd_card_rtk_capture_i2s_ops_multicap = {
    .open =         snd_card_capture_i2s_open_multicap,
    .close =        snd_card_capture_close_multicap,
    .ioctl =        snd_card_capture_ioctl,
    .hw_params =    snd_card_hw_params,
    .hw_free =      snd_card_capture_hw_free,
    .prepare =      snd_card_capture_prepare_multicap,
    .trigger =      snd_card_capture_trigger,
    .pointer =      snd_card_capture_pointer,
    .copy =         NULL,
    .silence =      NULL,
...
static snd_pcm_ops_t snd_card_rtk_capture_analog_ops = {
    ...
    .close =        snd_card_capture_close,
    .ioctl =        snd_card_analog_capture_ioctl,
    .hw_params =    snd_card_hw_params,
    .hw_free =      snd_card_capture_hw_free,
    .prepare =      snd_card_capture_prepare,
    .trigger =      snd_card_capture_trigger,
...
static int snd_realtek_hw_capture_init_LPCM_ringheader_of_AI_multicap(snd_pcm_runtime_t *runtime)
{
    snd_card_RTK_capture_pcm_t *dpcm = runtime->private_data;
    RINGBUFFER_HEADER *pAIRingHeader = &dpcm->nLPCMRing;
    RINGBUFFER_HEADER *pAIRingHeader_LE = &dpcm->nLPCMRing_LE;
...
static int snd_realtek_reprepare(snd_pcm_runtime_t *runtime) 
{
	...
	// AO stop
    if (RPC_TOAGENT_SETRINGBUFFER_NULL_SVC(dpcm->AOAgentID, dpcm->AOpinID)) {
        ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
        return -1; 
    }
...
static int snd_realtek_hw_capture_malloc_lpcm_ring_multicap(snd_pcm_runtime_t *runtime)
{
    //ALSA_VitalPrint("[ALSA %s %d]\n", __FUNCTION__, __LINE__);
    ...
    //printk("%s create enc_lpcm_handle_multicap\n", __FUNCTION__);
    enc_lpcm_handle_multicap = ion_alloc(alsa_client, RTK_ENC_LPCM_BUFFER_SIZE, 1024, RTK_PHOENIX_ION_HEAP_AUDIO_MASK, AUDIO_ION_FLAG);

    if (IS_ERR(enc_lpcm_handle_multicap)) {
        ALSA_WARNING("[%s %d ion_alloc fail]\n", __FUNCTION__, __LINE__);
        return bMallocSuccess;
    }

    if(ion_phys(alsa_client, enc_lpcm_handle_multicap, &dat, &len) != 0)
    {
        printk("[malloc fail and terminate %s %d]\n", __FUNCTION__, __LINE__);
        return bMallocSuccess;
    }
    ...
    dpcm->phy_pLPCMData= dat;
    dpcm->pLPCMData = ion_map_kernel(alsa_client, enc_lpcm_handle_multicap);
    ALSA_VitalPrint("[ALSA LPCM phy %x vir %p]\n", (unsigned int)dpcm->phy_pLPCMData, dpcm->pLPCMData);
#else
    void *addr = 0;
    addr = dma_alloc_coherent(NULL, RTK_ENC_LPCM_BUFFER_SIZE, &dpcm->phy_pLPCMData,  GFP_KERNEL);
...
static int snd_realtek_hw_capture_free_ring_multicap (snd_pcm_runtime_t *runtime)
{
    snd_card_RTK_capture_pcm_t *dpcm = runtime->private_data;
    ...
    {
#ifdef USE_ION_AUDIO_HEAP
        if(alsa_client != NULL && enc_lpcm_handle_multicap != NULL)
        {
            ion_unmap_kernel(alsa_client, enc_lpcm_handle_multicap);
            ion_free(alsa_client, enc_lpcm_handle_multicap);
            enc_lpcm_handle_multicap = NULL;
        }
#else
        dma_free_coherent(NULL, RTK_ENC_LPCM_BUFFER_SIZE, dpcm->pLPCMData, dpcm->phy_pLPCMData);
#endif
...
static void snd_card_capture_multicap_runtime_free(snd_pcm_runtime_t *runtime)
{
    ALSA_VitalPrint("[ALSA %s %d]\n", __FUNCTION__, __LINE__);
#ifndef USE_ION_AUDIO_HEAP
    snd_card_RTK_capture_pcm_t *dpcm = runtime->private_data;
    ...
    //snd_realtek_hw_capture_free_ring(runtime);
#ifdef USE_ION_AUDIO_HEAP
    if(alsa_client != NULL && alsa_capture_handle_multicap != NULL)
    {
        //printk("%s free alsa_capture_handle_multicap\n", __FUNCTION__);
        ion_unmap_kernel(alsa_client, alsa_capture_handle_multicap);
        ion_free(alsa_client, alsa_capture_handle_multicap);
        alsa_capture_handle_multicap = NULL;
    }
#else
    dma_free_coherent(NULL, 4096, dpcm, dpcm->phy_addr);
#endif
...
//    ALSA_VitalPrint("[ALSA %s %s %d %s]\n",substream->pcm->name, __FUNCTION__, __LINE__, __TIME__);
    ...
    alsa_capture_handle_multicap = ion_alloc(alsa_client, sizeof(snd_card_RTK_capture_pcm_t), 1024, RTK_PHOENIX_ION_HEAP_AUDIO_MASK, AUDIO_ION_FLAG);

    if (IS_ERR(alsa_capture_handle_multicap)) {
        ALSA_WARNING("[%s %d ion_alloc fail]\n", __FUNCTION__, __LINE__);
        goto fail;
    }

    if(ion_phys(alsa_client, alsa_capture_handle_multicap, &dat, &len) != 0) {
        printk("[%s %d] alloc memory faild\n", __FUNCTION__, __LINE__);
        goto fail;
    }

    dpcm = ion_map_kernel(alsa_client, alsa_capture_handle_multicap);
    memset(dpcm, 0, len);
    dpcm->phy_addr = dat;
#else
    dma_addr_t dat;
    ...
    // private data
    runtime->private_data = dpcm;
    runtime->private_free = snd_card_capture_multicap_runtime_free;
...
static int snd_card_capture_close_multicap(snd_pcm_substream_t * substream)
{
	...
	{
		// free ring_ext
		if (RPC_TOAGENT_SETRINGBUFFER_NULL_SVC(dpcm->AIAgentID, EXT_BS_OUT)) {
			ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
			goto exit; 
		}
		if (RPC_TOAGENT_AI_DISCONNECT_ALSA_MULTICAP(dpcm->AIAgentID))
		{
			ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
			goto exit;
		}
...
exit:
    ...
    snd_realtek_hw_capture_free_ring_multicap(runtime);
...
static int snd_card_analog_capture_ioctl(struct snd_pcm_substream *substream,  unsigned int cmd, void *arg)
{
    ...
    switch (cmd) 
    {
        case SNDRV_PCM_IOCTL_VOLUME_SET:
			volume = (unsigned int *)arg;
			return RPC_TOAGENT_SET_AI_FLASH_VOLUME(dpcm, *volume);
            break;
        case SNDRV_PCM_IOCTL_VOLUME_GET:
            break;
        case SNDRV_PCM_IOCTL_GET_FW_DELAY:
...
static int snd_card_capture_prepare_32bits_BE_multicap(snd_pcm_substream_t * substream)
{
    snd_pcm_runtime_t *runtime = substream->runtime;
    snd_card_RTK_capture_pcm_t *dpcm = runtime->private_data;
...
static int snd_card_capture_prepare_LPCM_multicap(snd_pcm_substream_t * substream)
{
    snd_pcm_runtime_t *runtime = substream->runtime;
    snd_card_RTK_capture_pcm_t *dpcm = runtime->private_data;
    ...
    // malloc LPCM ring buf
    if(snd_realtek_hw_capture_malloc_lpcm_ring_multicap(runtime))
    {
        ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
        return -ENOMEM;
    }
    ...
    // init LPCM ring header of AI
    if(snd_realtek_hw_capture_init_LPCM_ringheader_of_AI_multicap(runtime))
    {
        ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
        return -ENOMEM;
    }
    ...
    // private info
    if(RPC_TOAGENT_AI_CONNECT_ALSA_MULTICAP(runtime))
    {
        ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
        return -ENOMEM;
    }
	
    dpcm->nStartTimer.function = (void (*) (unsigned long))snd_card_capture_lpcm_timer_function_multicap;
...
//        ALSA_CAPTURE_TODO();
    ...
    switch(dpcm->nAIFormat)
    {
        case AUDIO_ALSA_FORMAT_32BITS_BE_PCM:
            ALSA_WARNING("ALSA: unsupport @ %s %d\n", __FUNCTION__, __LINE__);
            if(snd_card_capture_prepare_32bits_BE_multicap(substream))
            {
                ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
                return -ENOMEM;
            }
        ...
        case AUDIO_ALSA_FORMAT_16BITS_LE_LPCM:
        case AUDIO_ALSA_FORMAT_24BITS_LE_LPCM:
            if(snd_card_capture_prepare_LPCM_multicap(substream))
            {
                ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
                return -ENOMEM;
            }
...
int capture_substreams)
{
	...
	switch(instance_idx)
	{
		...
		case 3:
			...
			break;
		case 4:
			snd_pcm_set_ops(pcm, SNDRV_PCM_STREAM_CAPTURE, &snd_card_rtk_capture_i2s_ops_multicap);
			sprintf(pcm->name, SND_REALTEK_DRIVER_I2S_IN_multicap);
			break;
		default:
		ALSA_WARNING("[%s %d fail]\n", __FUNCTION__, __LINE__);
		break;
...
#if 1 // __LINUX_MEDIA_NAS__
void snd_realtek_hw_playback_switch_hdmi_work(struct work_struct *work) {
    RTK_snd_card_t *mars = container_of(work, RTK_snd_card_t, work_switch_hdmi);
    TRACE_CODE("[���� %s %d]\n", __FUNCTION__, __LINE__);
    RPC_TOAGENT_SWITCH_OUTPUT(ENUM_AUDIO_OUTPUT_SWITCH_HDMI, mars->switch_hdmi);
}

void snd_realtek_hw_playback_switch_i2s_work(struct work_struct *work) {
    RTK_snd_card_t *mars = container_of(work, RTK_snd_card_t, work_switch_i2s);
    TRACE_CODE("[���� %s %d]\n", __FUNCTION__, __LINE__);
    RPC_TOAGENT_SWITCH_OUTPUT(ENUM_AUDIO_OUTPUT_SWITCH_I2S, mars->switch_i2s);
}

void snd_realtek_hw_playback_switch_dac_work(struct work_struct *work) {
    RTK_snd_card_t *mars = container_of(work, RTK_snd_card_t, work_switch_dac);
    TRACE_CODE("[���� %s %d]\n", __FUNCTION__, __LINE__);
    RPC_TOAGENT_SWITCH_OUTPUT(ENUM_AUDIO_OUTPUT_SWITCH_DAC, mars->switch_dac);
}

void snd_realtek_hw_playback_switch_spdif_work(struct work_struct *work) {
    RTK_snd_card_t *mars = container_of(work, RTK_snd_card_t, work_switch_spdif);
    TRACE_CODE("[���� %s %d]\n", __FUNCTION__, __LINE__);
    RPC_TOAGENT_SWITCH_OUTPUT(ENUM_AUDIO_OUTPUT_SWITCH_SPDIF, mars->switch_spdif);
}
#endif
...
#if 1 // __LINUX_MEDIA_NAS__
    pRTK_card->switch_hdmi = 1;
    pRTK_card->switch_i2s = 1;
    pRTK_card->switch_dac = 1;
    pRTK_card->switch_spdif = 1;
    INIT_WORK(&pRTK_card->work_switch_hdmi, snd_realtek_hw_playback_switch_hdmi_work);
    INIT_WORK(&pRTK_card->work_switch_i2s, snd_realtek_hw_playback_switch_i2s_work);
    INIT_WORK(&pRTK_card->work_switch_dac, snd_realtek_hw_playback_switch_dac_work);
    INIT_WORK(&pRTK_card->work_switch_spdif, snd_realtek_hw_playback_switch_spdif_work);
#endif
...
static int snd_RTK_switch_hdmi_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    //printk("[ALSA %s %d] switch=%d\n", __FUNCTION__, __LINE__, mars->switch_hdmi);
    spin_lock_irqsave(&mars->mixer_lock, flags);
    ucontrol->value.integer.value[0] = mars->switch_hdmi;
    spin_unlock_irqrestore(&mars->mixer_lock, flags);
    return 0;
}
...
static int snd_RTK_switch_hdmi_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    unsigned long flags;
    int change = 0;
    //printk("[ALSA %s %d] switch=%d set=%ld\n", __FUNCTION__, __LINE__, mars->switch_hdmi, ucontrol->value.integer.value[0]);

    spin_lock_irqsave(&mars->mixer_lock, flags);
    if (mars->switch_hdmi != ucontrol->value.integer.value[0]) {
        mars->switch_hdmi = ucontrol->value.integer.value[0];
        change = 1;
    }
    spin_unlock_irqrestore(&mars->mixer_lock, flags);

    schedule_work(&mars->work_switch_hdmi);
...
static int snd_RTK_switch_i2s_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    //printk("[ALSA %s %d] switch=%d\n", __FUNCTION__, __LINE__, mars->switch_i2s);
    spin_lock_irqsave(&mars->mixer_lock, flags);
    ucontrol->value.integer.value[0] = mars->switch_i2s;
    spin_unlock_irqrestore(&mars->mixer_lock, flags);
    return 0;
}
...
static int snd_RTK_switch_i2s_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    unsigned long flags;
    int change = 0;
    //printk("[ALSA %s %d] switch=%d set=%ld\n", __FUNCTION__, __LINE__, mars->switch_i2s, ucontrol->value.integer.value[0]);

    spin_lock_irqsave(&mars->mixer_lock, flags);
    if (mars->switch_i2s != ucontrol->value.integer.value[0]) {
        mars->switch_i2s = ucontrol->value.integer.value[0];
        change = 1;
    }
    spin_unlock_irqrestore(&mars->mixer_lock, flags);

    schedule_work(&mars->work_switch_i2s);
...
static int snd_RTK_switch_dac_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    spin_lock_irqsave(&mars->mixer_lock, flags);
    ucontrol->value.integer.value[0] = mars->switch_dac;
    spin_unlock_irqrestore(&mars->mixer_lock, flags);
    return 0;
}
...
static int snd_RTK_switch_dac_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    spin_lock_irqsave(&mars->mixer_lock, flags);
    if (mars->switch_dac != ucontrol->value.integer.value[0]) {
        mars->switch_dac = ucontrol->value.integer.value[0];
        change = 1;
    }
    spin_unlock_irqrestore(&mars->mixer_lock, flags);

    schedule_work(&mars->work_switch_dac);
...
static int snd_RTK_switch_spdif_get(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    //printk("[ALSA %s %d] switch=%d\n", __FUNCTION__, __LINE__, mars->switch_spdif);
    spin_lock_irqsave(&mars->mixer_lock, flags);
    ucontrol->value.integer.value[0] = mars->switch_spdif;
    spin_unlock_irqrestore(&mars->mixer_lock, flags);
    return 0;
}
...
static int snd_RTK_switch_spdif_put(snd_kcontrol_t * kcontrol, snd_ctl_elem_value_t * ucontrol)
{
    ...
    unsigned long flags;
    int change = 0;
    //printk("[ALSA %s %d] switch=%d set=%ld\n", __FUNCTION__, __LINE__, mars->switch_spdif, ucontrol->value.integer.value[0]);

    spin_lock_irqsave(&mars->mixer_lock, flags);
    if (mars->switch_spdif != ucontrol->value.integer.value[0]) {
        mars->switch_spdif = ucontrol->value.integer.value[0];
        change = 1;
    }
    spin_unlock_irqrestore(&mars->mixer_lock, flags);

    schedule_work(&mars->work_switch_spdif);

    return change;
}
#endif // __LINUX_MEDIA_NAS__
...
static int snd_card_probe(struct platform_device *devptr)
{
	...
	// create another instance for capture AI I2S in foreground
