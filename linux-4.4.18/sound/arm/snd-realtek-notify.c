...
static const struct of_device_id rtk_ai_matches[] = {
	{.compatible = "alc5279",   .data = U32_PTR(ENUM_DT_AI_ADC)},
	{.compatible = "wm8782",    .data = U32_PTR(ENUM_DT_AI_ADC)},
	{.compatible = "adc",       .data = U32_PTR(ENUM_DT_AI_ADC)},
	{.compatible = "analog-in", .data = U32_PTR(ENUM_DT_AI_ANALOG_IN)},
	{.compatible = "adc-amic",  .data = U32_PTR(ENUM_DT_AI_ADC_AMIC)},
	{}
};
	...
	struct rpc_data *rpcd)
{
	...
	if (of_device_is_compatible(np, "realtek,audio-analog-in"))
		val = ENUM_DT_AI_ANALOG_IN;
	else {
		match = of_match_node(rtk_ai_matches, np);
		if (!match)
			val = ENUM_DT_AI_ADC;
		else
			val = PTR_U32(match->data);
	}

	dev_dbg(dev, "device type id: %d\n", val);
	rpcd->data[AN_AI_DEVICE_ON] |= BIT(val);
...
static struct rpc_data ai_data = {
	...
	.data = {
		[AN_AI_DEVICE_ON] = BIT(ENUM_DT_AI_AIN),
	},
	.size = 2,
	...
	struct rpc_data *rpcd)
{
	...
	for_each_child_of_node(np, child) {
		...
		dev_dbg(dev, "update mask to 0x%08x by %s\n", mask, match->compatible);
	}
...
static const struct of_device_id rtk_audio_dev_ids[] = {
	...
	{.compatible = "realtek,audio-analog-in", .data = &ai_data},
