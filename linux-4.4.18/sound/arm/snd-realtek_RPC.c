...
int RPC_TOAGENT_SETRINGBUFFER_NULL_SVC(int agentID, int pinID) 
{
    ...
    rpc = ion_map_kernel(alsa_client, handle);
    rpc->header.instanceID = htonl(agentID);
    rpc->header.pinID = htonl(pinID);
...
{
    AUDIO_RPC_AIO_PRIVATEINFO_PARAMETERS *rpc;
    AUDIO_RPC_PRIVATEINFO_RETURNVAL*res;
    HRESULT RPC_ret;
    int ret = -1;
    struct ion_handle *handle = NULL;
	...
	rpc->argateInfo[0] = htonl(ENUM_AI_PRIVATE_WRITE_BACK_SYSTEM_EXT);
    rpc->argateInfo[1] = htonl(AUDIO_ALSA_FORMAT_NONE);
	rpc->argateInfo[2] = htonl(0);
...
int RPC_TOAGENT_SWITCH_OUTPUT(AUDIO_OUTPUT_SWITCH output, int value)
{
    ...
    TRACE_RPC();
    TRACE_CODE("[%s %s %d] output=%d value=%d\n", __FILE__, __FUNCTION__, __LINE__, output, value);
    handle = ion_alloc(alsa_client, 4096, 1024, RTK_PHOENIX_ION_HEAP_AUDIO_MASK, AUDIO_ION_FLAG);
    ...
    switch (output)
    {
        case ENUM_AUDIO_OUTPUT_SWITCH_HDMI:
            config->msgID = htonl(AUDIO_CONFIG_CMD_HDMI_OUTPUT_SWITCH);
            break;
        case ENUM_AUDIO_OUTPUT_SWITCH_I2S:
        case ENUM_AUDIO_OUTPUT_SWITCH_DAC:
            config->msgID = htonl(AUDIO_CONFIG_CMD_DAC_OUTPUT_SWITCH);
            break;
        case ENUM_AUDIO_OUTPUT_SWITCH_SPDIF:
            config->msgID = htonl(AUDIO_CONFIG_CMD_SPDIF_OUTPUT_SWITCH);
            break;
        default:
            goto exit;
    }
    if (value == 0) {
        // mute
        if (output == ENUM_AUDIO_OUTPUT_SWITCH_I2S)
            config->value[0] = htonl(2);
        else if (output == ENUM_AUDIO_OUTPUT_SWITCH_DAC)
            config->value[0] = htonl(4);
        else
            config->value[0] = htonl(0);
    } else {
        // unmute
        if (output == ENUM_AUDIO_OUTPUT_SWITCH_I2S)
            config->value[0] = htonl(3);
        else if (output == ENUM_AUDIO_OUTPUT_SWITCH_DAC)
            config->value[0] = htonl(5);
        else
...
int RPC_TOAGENT_AI_CONNECT_ALSA_MULTICAP(snd_pcm_runtime_t *runtime)
{
	...
	rpc->type = htonl(ENUM_PRIVATEINFO_AIO_AI_PRIVATEINFO);
	rpc->argateInfo[0] = htonl(ENUM_AI_PRIVATE_WRITE_BACK_SYSTEM_EXT);
    rpc->argateInfo[1] = htonl(dpcm->nAIFormat);
	rpc->argateInfo[2] = htonl(runtime->rate);
