...
#define RTK_DMP_CAPTURE_MAX_BUFFER_SIZE         RTK_DMP_PLAYBACK_MAX_BUFFER_SIZE*4 // 32*1024 is for max 2 channels, max 8 channels need to multiply 4
#define RTK_DMP_CAPTURE_MIN_PERIOD_BYTE         RTK_DMP_PLAYBACK_MIN_PERIOD_SIZE
#define RTK_DMP_CAPTURE_MAX_PERIOD_BYTE         4096*4  // 4096 is for max 2 channels, max 8 channels need to multiply 4
...
#define MARS_SWITCH_HDMI(xname, xindex, addr)    \
{ .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xindex, \
  .info = snd_RTK_switch_hdmi_info, \
  .get = snd_RTK_switch_hdmi_get, .put = snd_RTK_switch_hdmi_put, \
  .private_value = addr }

#define MARS_SWITCH_I2S(xname, xindex, addr)    \
{ .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xindex, \
  .info = snd_RTK_switch_i2s_info, \
  .get = snd_RTK_switch_i2s_get, .put = snd_RTK_switch_i2s_put, \
  .private_value = addr }

#define MARS_SWITCH_DAC(xname, xindex, addr)    \
{ .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xindex, \
  .info = snd_RTK_switch_dac_info, \
  .get = snd_RTK_switch_dac_get, .put = snd_RTK_switch_dac_put, \
  .private_value = addr }

#define MARS_SWITCH_SPDIF(xname, xindex, addr)    \
{ .iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, .index = xindex, \
  .info = snd_RTK_switch_spdif_info, \
  .get = snd_RTK_switch_spdif_get, .put = snd_RTK_switch_spdif_put, \
  .private_value = addr }
#endif
...
#if 1 // __LINUX_MEDIA_NAS__
typedef enum {
    ENUM_AUDIO_OUTPUT_SWITCH_HDMI,
    ENUM_AUDIO_OUTPUT_SWITCH_I2S,
    ENUM_AUDIO_OUTPUT_SWITCH_DAC,
    ENUM_AUDIO_OUTPUT_SWITCH_SPDIF,
} AUDIO_OUTPUT_SWITCH;
#endif
...
#if 1 // __LINUX_MEDIA_NAS__
    int switch_hdmi;    /* 0 as mute */
    int switch_i2s;
    int switch_dac;
    int switch_spdif;
    struct work_struct work_switch_hdmi;
    struct work_struct work_switch_i2s;
    struct work_struct work_switch_dac;
    struct work_struct work_switch_spdif;
#endif
} RTK_snd_card_t;
...
int RPC_TOAGENT_SETRINGBUFFER_NULL_SVC(int agentID, int pinID);
...
#if 1 // __LINUX_MEDIA_NAS__
int RPC_TOAGENT_SWITCH_OUTPUT(AUDIO_OUTPUT_SWITCH output, int value);
#endif
...
int RPC_TOAGENT_AI_CONNECT_ALSA_MULTICAP(snd_pcm_runtime_t *runtime);
int RPC_TOAGENT_AI_DISCONNECT_ALSA_MULTICAP(int instance_id);
