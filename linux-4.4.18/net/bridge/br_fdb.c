...
{
#if defined(CONFIG_RTL_IGMP_SNOOPING)
	bool rtnl_key_acquired = FALSE;
#endif

#if defined(CONFIG_RTL_IGMP_SNOOPING)
	if (f->is_static) {
		/* mutex_trylock() must not be used inside interrupt context
		 * since its implementation is not safe for that.
		 * Nevertheless, rtl8197 HW MCAST needs it unless change its architecture.
		 * */
		if (rtnl_trylock())
			rtnl_key_acquired = TRUE;
		fdb_del_hw_addr(br, f->addr.addr);

		if (rtnl_key_acquired)
			rtnl_unlock();
	}
#else
	if (f->is_static)
