...
#ifndef CONFIG_CRYPTO_DEV_RTK_MCP_INTERRUPT
#define DESC_LEN  10
#else
#define ASYNC_NAPI
#define COPY_CTX
...
int mcp_do_command(mcp_desc* p_desc, int n_desc, uint32_t key_len, char *key, uint32_t mode, struct rtk_aes_ctx * ctx);
...
#ifdef CONFIG_CRYPTO_DEV_RTK_MCP_INTERRUPT
...
static int rtk_handler(void *data)
{
    ...
    do {
        ...
        unsigned int sg_in_count, sg_out_count;
        struct crypto_async_request *handled_req = NULL;
        struct scatterlist* src, *dst;
        struct ablkcipher_request *req_tmp;
        unsigned int i=0;
        ...
        if (count) {
            ...
            while (1)
            {
                spin_lock_irq(&dd->lock);
                req_tmp=NULL;
                handled_req = crypto_dequeue_request(&dd->queue);
                if (handled_req == NULL) {
                        spin_unlock_irq(&dd->lock);
                        break;
                }
                ...
                spin_unlock_irq(&dd->lock);
                req_tmp = ablkcipher_request_cast(handled_req);
                if (req_tmp)
                {
                    src = req_tmp->src;
                    dst = req_tmp->dst;
                    ...
                    req_tmp->base.complete(&(req_tmp->base), 0);
                }
                spin_lock_irq(&dd->lock);
...
static int mcp_probe(struct platform_device *pdev)
{
    ...
    cpioaddr =  of_iomap(node, 0);
    tpioaddr =  of_iomap(node, 1);
    if (mcp_init()<0)
        return -ENODEV;
    ...
    mcp_dd->desc=(mcp_desc*)(((unsigned long)(desc_list)+7)&(~0x7));
...
//    struct rtk_aes_ctx *ctx_list;
    ...
    mcp_dd->ctx = (struct rtk_aes_ctx*)(((unsigned long)(ctx_list)+7)&(~0x7));
#endif
    mcp_dd->aes_req =NULL;
...
//    struct device *dev = &pdev->dev;
    struct rtk_sha_dev *mcp_dd;
    mcp_dd = platform_get_drvdata(pdev);
...
//	pr_err("rtk_mcp_sg sg_in_anchor is 0x%x, sg_out_anchor is 0x%x\n", sg_in_anchor, sg_out_anchor);
    ...
    ret = mcp_do_command(desc, desc_total, ctx->keylen, ctx->key, mode, ctx);
