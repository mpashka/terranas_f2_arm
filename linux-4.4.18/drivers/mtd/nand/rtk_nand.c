...
#define VERSION  "$Id: rtk_nand.c 2018-09-20 03:45:30Z kentseng $"
...
//#define rtd_outl(offset,val)    (*(volatile unsigned long *)(offset) = val); asm ("sync")
...
static void rtk_nand_shutdown(struct platform_device *pdev)
{
	rtk_nand_suspend(&pdev->dev);
}
//----------------------------------------------------------------------------
...
static SIMPLE_DEV_PM_OPS(rtk_nand_pm_ops, rtk_nand_suspend, rtk_nand_resume);
#endif
//----------------------------------------------------------------------------
static void rtk_read_oob_from_SRAM(u_char *r_oobbuf)
{
	...
	reg = ioremap(0x18010000+RBUS_OFFSET, 0x100);
	...
	while (1) 
	{
		...
		if ((blank_check & 0xa) != 0x8) {
			...
			if (REG_READ_U32(REG_ND_ECC+map_base) & 0x04){
				...
				// Suppress 1-bit ECC corrected error message
				if(rc > 1){
