...
static const struct edid_product_info_t product_info[] = {
	{.mfg_name[0]=0x2E, .mfg_name[1]=0x83, .prod_code[0]=0x00, .prod_code[1]=0x55, .serial=1, \
	 .vic_mask=0xFFFFFFFFFFFFFFFF, .vic2_mask=0x0,.extended_vic_mask=0x0, .vic2_420_mask=0x0},
};
...
int rtk_filter_specific_profuct_vic(struct edid *edid, struct sink_capabilities_t *sink_cap)
{
	...
	{
		if((edid->mfg_id[0]==product_info[i].mfg_name[0])&&(edid->mfg_id[1]==product_info[i].mfg_name[1]) && \
			(edid->prod_code[0]==product_info[i].prod_code[0])&&(edid->prod_code[1]==product_info[i].prod_code[1]) && \
