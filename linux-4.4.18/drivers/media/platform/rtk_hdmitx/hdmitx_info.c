...
const VO_STANDARD_TABLE_T vo_standard_table[] = {
...
{
	unsigned int ret_count,vo_standard;
	...
	char *depth[] = { "24 Bits", "30 Bits", "36 Bits"};
	...
	char *format3d[] = {"Auto","FramePacking","Side-by-Side","Top-and-Bottom"};
	char *yes_no[] = {"No","Yes"};
	...
	ret_count += sprintf(buf+ret_count, "Resolution: %ux%u%s @ %uHz\n",vo_standard_table[vo_standard].width,
															  ...
															  vo_standard_table[vo_standard].interlace?"I":"P",
	...
	if(depp_depth==5)//30 bits
		ret_count += sprintf(buf+ret_count, "Color depth: %s\n", depth[1]);
	else if(depp_depth==6)//36 bits
		ret_count += sprintf(buf+ret_count, "Color depth: %s\n", depth[2]);
	else
		ret_count += sprintf(buf+ret_count, "Color depth: %s\n", depth[0]);
