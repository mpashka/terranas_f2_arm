...
unsigned char hdmitx_send_scdc_TmdsConfig(unsigned int standard, unsigned int dataInt0)
{
	...
	switch(standard)
	{
		...
		case VO_STANDARD_HDTV_4096_2160P_60_3D:
		{
			switch(format_3D)
			{
				...
				{
					if((standard==VO_STANDARD_HDTV_2160P_50_3D)||(standard==VO_STANDARD_HDTV_2160P_60_3D)||
						(standard==VO_STANDARD_HDTV_4096_2160P_50_3D)||(standard==VO_STANDARD_HDTV_4096_2160P_60_3D))
	...
	if(hdmitx_edid_info.scdc_capable&SCDC_PRESENT)
	{
		...
		HDMI_INFO("Send SCDC TMDS_Config(0x%02x), deep_color(%u) 3D(%u)",config_data,deep_color,format_3D);
	}
	...
	else if((standard>=VO_STANDARD_HDTV_2160P_60_420)&&(standard<=VO_STANDARD_HDTV_4096_2160P_50_420))// 4K YUV420
	{
