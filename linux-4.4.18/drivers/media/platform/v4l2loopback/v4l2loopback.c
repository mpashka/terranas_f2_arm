...
extern uint32_t vfb_phyaddr;
extern struct ion_handle *vfb_handle;
...
struct v4l2_loopback_device {
	...
	struct mutex mutex_lock;		/* mutex for open/mmap... */
	int set_ion;
};
...
#define FORMAT_FLAGS_SEMI_PLANAR  0x03
...
/* query the physical address */
#define V4L2_S_ION_ADDRESS	_IO ('v', BASE_VIDIOC_PRIVATE+0)
/* query the physical address */
/*#define V4L2_G_PHYSICAL_ADDRESS	_IOR ('v', BASE_VIDIOC_PRIVATE+0, int)*/
/* set the write position */
#define V4L2_S_WRITE_POS	_IOW ('v', BASE_VIDIOC_PRIVATE+1, int)
/* get the read position */
#define V4L2_G_READ_POS	_IOR ('v', BASE_VIDIOC_PRIVATE+2, int)
		...
		unsigned int width, unsigned int height)
{
	...
	if (fmt->flags & FORMAT_FLAGS_PLANAR) {
		...
		f->sizeimage = (width * ALIGN_16(height) * fmt->depth) >> 3;
		...
		f->sizeimage = ALIGN_16(height) * f->bytesperline;
	}
}
...
static int allocate_buffers(struct v4l2_loopback_device *dev, int set_ionaddr);
...
static int can_read(struct v4l2_loopback_device *dev, struct v4l2_loopback_opener *opener)
{
	...
	spin_lock_bh(&dev->lock);
	check_timers(dev);
	ret1 = dev->write_position > opener->read_position
		|| dev->reread_count > opener->reread_count
		|| dev->timeout_happened;
	for (i=0 ; i < dev->buffers_number; i++)
	{
		if ((dev->buffers[i].buffer.flags&V4L2_BUF_FLAG_QUEUED)!=0)
		{
			ret2 = 1;
			break;
		}
...
static int vidioc_dqbuf(struct file *file, void *private_data, struct v4l2_buffer *buf)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		//dprintk("capture DQBUF pos: %d index: %d\n", opener->read_position - 1, index);
		...
		*buf = dev->buffers[index].buffer;
		dev->read_position = opener->read_position;
		dev->cur_read_pos = index;
		return 0;
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
...
static int v4l2_set_ion_address(struct file *file, void *fh)
{
	struct v4l2_loopback_device *dev;
	int ret;
	MARK();
...
static int v4l2_set_write_position(struct file *file, void *fh, int *pos)
{
	struct v4l2_loopback_device *dev;
	int write_index;
	...
	b = &dev->buffers[*pos].buffer;

	do_gettimeofday(&b->timestamp);
	b->bytesused = dev->pix_format.sizeimage;
	b->sequence = dev->write_position;
	buffer_written(dev, &dev->buffers[*pos]);
	wake_up_all(&dev->read_event);
	//wake_up_interruptible(&dev->proc_list);
	dprintkrw("leave v4l2_loopback_write()\n");
	return 0;
}

static int v4l2_get_read_position(struct file *file, void *fh, int *pos)
{
	struct v4l2_loopback_device *dev;
	MARK();

	dev = v4l2loopback_getdevice(file);
	*pos = dev->cur_read_pos;
...
{
	switch (cmd) {
	case V4L2_S_ION_ADDRESS:
		return v4l2_set_ion_address(file, fh);

	case V4L2_S_WRITE_POS:
		return v4l2_set_write_position(file, fh, (int *) arg);

	case V4L2_G_READ_POS:
		return v4l2_get_read_position(file, fh, (int *) arg);
...
static int v4l2_loopback_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (NULL == dev->image)
	{
		dprintk("mmap fail image null\n");
		if (allocate_buffers(dev, 0) < 0)
			return -EINVAL;
	}
	if (opener->timeout_image_io) {
	...
	MARK();
	return ion_mmap_by_handle(vfb_handle,vma);
}
...
static int allocate_buffers(struct v4l2_loopback_device *dev, int set_ionaddr)
{
	...
	if (dev->image) {
		...
		if (dev->buffer_size * dev->buffers_number == dev->imagesize && set_ionaddr == 0)
			return 0;
		...
		{
			free_buffers(dev);
			dprintk("free allocated buffers\n");
		}
		else
			return -EINVAL;
	}
	...
	{
		dev->set_ion = 1;
		dev->image = __va(vfb_phyaddr);
		dprintk("[%s] ion buffer vAddr:%p\n",__func__, dev->image);
	}
	else
	{
...
static int v4l2_loopback_init(struct v4l2_loopback_device *dev, int nr)
{
	...
	if(ret)
		goto error;
	v4l2_ctrl_new_std(hdl, &v4l2loopback_ctrl_ops,V4L2_CID_BRIGHTNESS, 0, 63, 1, 32);
	v4l2_ctrl_new_std(hdl, &v4l2loopback_ctrl_ops,V4L2_CID_HUE, 0, 63, 1, 32);
	v4l2_ctrl_new_std(hdl, &v4l2loopback_ctrl_ops,V4L2_CID_CONTRAST, 0, 63, 1, 32);
	v4l2_ctrl_new_std(hdl, &v4l2loopback_ctrl_ops,V4L2_CID_SATURATION, 0, 63, 1, 32);
	v4l2_ctrl_new_std(hdl, &v4l2loopback_ctrl_ops,V4L2_CID_MIN_BUFFERS_FOR_CAPTURE, max_openers, MAX_BUFFERS, 1, max_buffers);
	...
	dev->buffer_size = PAGE_ALIGN(dev->pix_format.sizeimage);
	dprintk("buffer_size = %ld (=%d) PAGE_SIZE:%d\n", dev->buffer_size, dev->pix_format.sizeimage, PAGE_SIZE);
	allocate_buffers(dev, 0);
