...
#include <soc/realtek/rtd129x_reglock.h>
...
DEFINE_SPINLOCK(rtk_clk_en_lock);
EXPORT_SYMBOL_GPL(rtk_clk_en_lock);
...
static inline int rtk_clk_pm_store(void)
{
    ...
    list_for_each(it, &pm_data_list) {
        ...
        if ((generic->type & CLK_IS_RAW) != CLK_IS_RAW) {
            ...
            rtk_clk_en_lock_irqsave(flags);
            priv->value = readl(priv->reg);
            rtk_clk_en_unlock_irqrestore(flags);
