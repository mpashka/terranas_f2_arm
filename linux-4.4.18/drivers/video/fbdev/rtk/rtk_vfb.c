        ...
        case FBIOSET_RESIZE              :
            {
                ...
                if (copy_from_user(&sNewSize, (void *)arg, sizeof(sNewSize)) == 0) {
                    rtk_vfb_set_size(fb, sNewSize.width, sNewSize.height, sNewSize.count, sNewSize.byte_per_pixel, 0);
                }
                gat_cmd = 1;
                break;
            }
...
static int rtk_vfb_memory_alloc(struct rtk_vfb_memory ** ppMem, int width, int height, int byte_per_pixel ,int align, int count, int flags)
{
    uint32_t video_size = MEMORY_ALIGN((width * height * byte_per_pixel * count), align);
    ...
    *ppMem = (struct rtk_vfb_memory *) kzalloc(sizeof(struct rtk_vfb_memory), GFP_KERNEL);
    pMem = *ppMem;
	...
	pMem->fb_handle         = ion_alloc(pMem->fb_client, pMem->alloc_size, align, ion_heap_mask, ion_heap_flag);
    pMem->width             = width;
    pMem->height            = height;
    pMem->align             = align;
    pMem->count             = count;
...
static int rtk_vfb_set_size (struct rtk_vfb *fb, int width, int height, int count, int _byte_per_pixel, int flags)
{
    int ret = -1;
    struct rtk_vfb_memory * pMem = NULL;
    ...
    if (rtk_vfb_memory_alloc(&pMem, MEMORY_ALIGN(width,16), MEMORY_ALIGN(height,16), byte_per_pixel, PAGE_SIZE, count, flags) != 0) {
    ...
    if (rtk_vfb_update_by_memory(fb, pMem) != 0) {
...
static int rtk_vfb_probe(struct platform_device *pdev)
{
    ...
    if (rtk_vfb_set_size(fb, width, height, count, VIDEO_BYTES_PER_PIXEL, RTK_FB_MEM_ALLOC_ALGO_LAST_FIT) != 0) {
    ...
    //printk("%s %s %d I'm fd%d\n",__FILE__,__func__,__LINE__,my_fb_node);
#ifdef CONFIG_REALTEK_AVCPU
