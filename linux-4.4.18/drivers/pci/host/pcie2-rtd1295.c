...
static int rtk_pcie2_hw_initial(struct device *dev)
{
	...
	rtk_pci_ctrl_write(0xC1C, 0x27E90301);
	if(rtk_pcie_mdio_chk())
		return -1;
	...
	/* #modify AMP */
	rtk_pci_ctrl_write(0xC1C, 0xc4662001);
	if(rtk_pcie_mdio_chk())
		return -1;
	...
	rtk_pci_ctrl_write(0xC1C, 0x521C0901);
	if(rtk_pcie_mdio_chk())
		return -1;
	rtk_pci_ctrl_write(0xC1C, 0x501C0901);
	if(rtk_pcie_mdio_chk())
		return -1;
	rtk_pci_ctrl_write(0xC1C, 0x521C0901);
	if(rtk_pcie_mdio_chk())
		return -1;
	rtk_pci_ctrl_write(0xC1C, 0x08282401);
	if(rtk_pcie_mdio_chk())
		return -1;
	rtk_pci_ctrl_write(0xC1C, 0x08286401);
	if(rtk_pcie_mdio_chk())
		return -1;

	// after phy mdio set
	ret = gpio_direction_output(pcie1_gpio_reset, 0);
	mdelay(100);
	ret = gpio_direction_output(pcie1_gpio_reset, 1);
...
static int rtk_pcie2_probe(struct platform_device *pdev)
{
	...
	pcie1_gpio_reset = of_get_gpio_flags(pdev->dev.of_node, 0, NULL);
	if (gpio_is_valid(pcie1_gpio_reset)) {
		ret = gpio_request(pcie1_gpio_reset, "pcie1_gpio_reset");
		if (ret < 0)
			printk(KERN_ERR "%s: can't request gpio %d\n", __func__, pcie1_gpio_reset);
	} else
...
static int rtk_pcie2_remove(struct platform_device *pdev)
{
