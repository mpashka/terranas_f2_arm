...
#ifdef CONFIG_RTK_RPC
	rpc_ringbuf_base = (void *)of_iomap(np, 4);
	rpc_common_base = (void *)of_iomap(np, 1);
#endif /* CONFIG_RTK_RPC */
...
void __init rtk_mem_remap_of_init(void)
{
	...
	for_each_compatible_node(np, NULL, "Realtek,rtk1195-mem_remap"){
...
REALTEK_MAP_OF_DECLARE(realtek_memory_remap, "Realtek,rtk1195-mem_remap", rtk_mem_remap_init);
