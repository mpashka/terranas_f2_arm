...
struct _memory_verified_handle * memory_verified_handle_create(size_t byte)
{
    ...
    struct _memory_verified_handle * handle =
        (struct _memory_verified_handle *) kmalloc(sizeof(struct _memory_verified_handle), GFP_ATOMIC);
    ...
    if (byte > 0x8000)
        ...
        handle->memAddress = (char *) kmalloc(byte, GFP_ATOMIC);
...
struct _memory_verified_handle * memory_writeback_handle_create(struct _memory_verified_handle *phys_area)
{
    ...
    handle->memAddress = (char *)kmalloc(phys_area->memByte, GFP_ATOMIC);
    ...
    hexdump("[RTD129x_PM]Before Writeback Dump:", (unsigned char *) phys_to_virt((phys_addr_t)phys_area->memAddress), phys_area->memByte);

    memcpy(handle->memAddress, phys_to_virt((phys_addr_t)phys_area->memAddress), phys_area->memByte);
    __flush_dcache_area(handle->memAddress, phys_area->memByte);
out:
    return handle;
}

int memory_writeback_release(struct _memory_verified_handle *handle, struct _memory_verified_handle *phys_area)
{
    int ret = 0;
    ...
    hexdump("[RTD129x_PM]Resume back Dump:**********************", (unsigned char *) phys_to_virt((phys_addr_t)phys_area->memAddress), phys_area->memByte);	
    memcpy(phys_to_virt((phys_addr_t)phys_area->memAddress), handle->memAddress, handle->memByte);
    __flush_dcache_area(phys_to_virt((phys_addr_t)phys_area->memAddress), phys_area->memByte);
    hexdump("[RTD129x_PM]After Writeback Dump:", (unsigned char *) phys_to_virt((phys_addr_t)phys_area->memAddress), phys_area->memByte);
...
static int rtk_suspend_to_ram(void)
{
    ...
    memory_verified_handle_t *mem_wbhandle;
    ...
    mem_wbhandle = kmalloc(sizeof(phys_wb_area)/sizeof(struct _memory_verified_handle)*sizeof(memory_verified_handle_t*), GFP_ATOMIC);   
    if (mem_wbhandle) {	
    	for (i = 0; i < sizeof(phys_wb_area)/sizeof(struct _memory_verified_handle); i++) 
        	mem_wbhandle[i] = memory_writeback_handle_create(&phys_wb_area[i]);
    }	
    ...
    if(mem_wbhandle) { 
    	for (i = 0; i < sizeof(phys_wb_area)/sizeof(struct _memory_verified_handle); i++)
        	memory_writeback_release(mem_wbhandle[i], &phys_wb_area[i]);
