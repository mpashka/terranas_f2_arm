...
//#include <rbus/reg.h>
#include "../soc/realtek/rtd119x/include/reg_sb2.h"
...
static struct cpufreq_frequency_table realtek_1195_freq_table[] = {
	{ 0, 0, 600000 }, /* { flags, driver_data, frequency(kHz) } */
	{ 0, 1, 700000 },
	{ 0, 2, 800000 },
	{ 0, 3, 900000 },
	{ 0, 4,1000000 },
...
static int rtkSetVoltage(struct realtek_1195_dvfs * dvfs, unsigned int freqsnew)
{
	...
	if (dvfs->vddarm_min ==  900000) {
		newVal[0] = 0;
		newVal[1] = 0;
	} else if (dvfs->vddarm_min == 1050000) {
		newVal[0] = 0;
		newVal[1] = 1;
...
static int realtek_1195_update_cpu_speed(struct cpufreq_policy *policy, unsigned int idx)
{
	int ret = 0, result = 0;
	...
	dvfs = &realtek_1195_dvfs_table[realtek_1195_freq_table[idx].driver_data];
...
static void __init realtek_1195_cpufreq_config_regulator(void)
{
	...
	while (count > 0 && freq->frequency != CPUFREQ_TABLE_END) {
		...
		dvfs = &realtek_1195_dvfs_table[freq->driver_data];
		found = 0;
...
static int realtek_1195_cpufreq_driver_init(struct cpufreq_policy *policy)
{
	...
	ret = cpufreq_generic_init(policy, realtek_1195_freq_table, policy->cpuinfo.transition_latency);
    if (ret)
        return ret;
}
...
static struct cpufreq_driver realtek_1195_cpufreq_driver = {
	...
	.target_index 	= realtek_1195_update_cpu_speed,
...
static int rtk_dvfs_probe(struct platform_device *pdev)
{
	...
	if ((prop) && (!err)) {
		...
		pr_info("[%s] virtual-freq-threshold = %lu\n", __func__, virtual_freq_threshold);
	} else {
	...
	if ((prop) && (!err)) {
		...
		if (realtek_1195_freq_table == NULL) {
			...
			for (i=0;i<counter;i+=1) {
				freq->flags = 0;
				freq->driver_data = of_read_number(prop, 1 + (i*FREQUENCY_COLUMN));
...
#ifdef CONFIG_REGULATOR
	vddarm = regulator_get(NULL, "rtp5901-pmic");
//	if (!vddarm->supply_name)
//		pr_err("supply_name is NULL.\n");
	if (IS_ERR(vddarm)) {
		ret = PTR_ERR(vddarm);
...
#define TARGET_RANGE 200 //200kHz
			if (((freq->frequency + TARGET_RANGE/2) -r) < TARGET_RANGE) {
				pr_debug("[%s] SET freq->frequency %d => %lu\n",__func__,freq->frequency,r);
				freq->frequency = r;
			} else {
				pr_debug("%dkHz unsupported by clock (r = %lu)\n",
						freq->frequency,r);
				freq->frequency = CPUFREQ_ENTRY_INVALID;
			}
			freq++;
