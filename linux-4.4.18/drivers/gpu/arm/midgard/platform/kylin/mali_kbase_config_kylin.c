...
#define GPU_HW_SETTING

#ifdef GPU_HW_SETTING
...
static void pm_callback_power_off(struct kbase_device *kbdev)
{
    ...
    //u32 regValue;
#if 0//def GPU_HW_SETTING
    writel(0x7, rbus_base + GPU_POWER_OFF_CORES);
    writel(0x1, rbus_base + GPU_POWER_OFF_TILERS);
    writel(0x1, rbus_base + GPU_POWER_OFF_L2);
    //regValue = readl(rbus_base + GPU_STATUS_OF_CORES); //0x0000_0007
    //printk(KERN_ALERT "address[0x%x] = 0x%x (0x0007)\n", (RBUS_BASE + GPU_STATUS_OF_CORES), regValue);
    //regValue = readl(rbus_base + GPU_STATUS_OF_TILERS); //0x0000_0001
    //printk(KERN_ALERT "address[0x%x] = 0x%x (0x0001)\n", (RBUS_BASE + GPU_STATUS_OF_TILERS), regValue);
...
static void pm_callback_power_suspend(struct kbase_device *kbdev)
{
    ...
    if (kbdev->clock) {
        //printk(KERN_ALERT "[GPU] %s %d disable clock\n", __FUNCTION__, __LINE__);
...
int kbase_platform_early_init(void)
{
    ...
    //writel(0xFFFFFFFF, rbus_base + 0x1A308);
    //writel(0x0000FFFF, rbus_base + 0x1A30C);
    //writel(0x0000000F, rbus_base + 0x1A310);

    //////////////////////////////////////////////////////////////////////////////////////
    //memory set 0x980005A4 32 0xB212              # 682MHz
    //memory set 0x980005A4 32 0x9A12              # 601MHz
    //memory set 0x980005A4 32 0x8212              # 520MHz
    //memory set 0x980005A4 32 0x6A12              # 439MHz
    //memory set 0x980005A4 32 0x5212              # 357MHz
    ...
    //////////////////////////////////////////////////////////////////////////////////////
    //printk(KERN_ALERT "Clock Reset\n");
    regValue = readl(rbus_base + GPU_RESET);
    writel((regValue | 0x40000), rbus_base + GPU_RESET);
    regValue = readl(rbus_base + GPU_CLOCK);
    writel((regValue | 0x800), rbus_base + GPU_CLOCK);
