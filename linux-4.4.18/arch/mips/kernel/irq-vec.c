...
static int mips_vec_irq_map(struct irq_domain *d, unsigned int irq,
                             irq_hw_number_t hw)
{
        return 0;
}
...
static const struct irq_domain_ops mips_vec_irq_domain_ops = {
        .map = mips_vec_irq_map,
        .xlate = irq_domain_xlate_onecell,
};


void __init __mips_vec_irq_init(struct device_node *of_node)
{
	...
	write_lxc0_intvec(&handle_vec);
        ...
        domain = irq_domain_add_legacy(of_node, 8, MIPS_VEC_IRQ_BASE, 8,
                                       &mips_vec_irq_domain_ops, NULL);
        if (!domain)
                panic("Failed to add irqdomain for Taroko CPU");
...
void __init mips_vec_irq_init()
{
        __mips_vec_irq_init(NULL);
}

int __init mips_vec_irq_of_init(struct device_node *of_node,
...
IRQCHIP_DECLARE(taroko_vector, "realtek,taroko-vector", mips_vec_irq_of_init);
