...
my $MKUBIFS_GENERIC_PATH = "../bin/mkfs.ubifs_generic";  # for generic linux project.
...
sub get_partition_count
{
    ...
    #print "cmd1=$cmd1\n";
    my $partition_num = `cat $package_path/$customer_path/partition.txt | grep -e \"^part\" | wc -l`;
...
{
    my $partitions_info_tmp = `cat $package_path/$customer_path/partition.txt | grep -e \"^part\";cat $package_path/$customer_path/partition_customize.txt | grep -e \"^part\"`;
...
#        system("rm -rf $(CURDIR)/$(EXT4FS_SYSTEMIMG); sync"); 
	...
	else
	{
        ...
        system("$MAKE_EXT4FS -l $size_in_mb"."M -s $USED_FILE_CONTEXTS_PATH -a $label_name $tmp_path/s.bin $package_path/$partitions_by_labels{$label_name}{\"label\"}; sync;");
        my ($path, $ext4_img) = split /\//, $partitions_by_labels{$label_name}{"image_name"};
        system("$SIMG2IMG $tmp_path/s.bin $ext4_img; sync"); 
        system("$E2FSCK_PATH -f -p $ext4_img; sync;");
        system("$RESIZE2FS_PATH -M $ext4_img");
...
{
    if($layout_size eq "4gb_leanback")
    {
        $customer_path = "customer/rtk_generic_emmc_leanback";
    } elsif ($layout_size eq "4gb")
    {
