...
int usb_init(void)
{
	...
	for (i = 0; i < usb_max_controller_count; i++) {
		...
		else if(i == xhci_index)
		{
#if defined(CONFIG_USB_XHCI)
			if (xhci_usb_lowlevel_init(i, &ctrl)) {
				puts("xhci lowlevel init failed\n");
				continue;
			}
		...
		dev = usb_alloc_new_device(ctrl,attach_to_xhci);
		/*
...
int usb_stop(void)
{
	...
	if (usb_started) {
		...
		for (i = 0; i < usb_max_controller_count; i++) {
			...
			{
#if defined(CONFIG_USB_XHCI)
				if (xhci_usb_lowlevel_stop(i))
					printf("failed to stop xhci USB controller %d\n", i);
#else
				printf("failed to stop xhci USB controller %d\n", i);
#endif
			}
...
{
	if(dev->attach_to_xhci)
	{
#if defined(CONFIG_USB_XHCI)
		return xhci_submit_int_msg(dev, pipe, buffer, transfer_len, interval);
#else
		return -1;
#endif
			...
			void *data, unsigned short size, int timeout)
{
	...
	{
#if defined(CONFIG_USB_XHCI)
		if (xhci_submit_control_msg(dev, pipe, data, size, setup_packet) < 0)
			return -1;
#else
		return -1;
#endif
			...
			void *data, int len, int *actual_length, int timeout)
{
	...
	{
#if defined(CONFIG_USB_XHCI)
		if (xhci_submit_bulk_msg(dev, pipe, data, len) < 0)
			return -1;
#else
		return -1;
#endif
			...
			void *data, int len, int *actual_length, int timeout)
{
	...
	{
#if defined(CONFIG_USB_XHCI)
		if (xhci_submit_int_msg(dev, pipe, data, len, timeout) < 0)
			return -1;
#else
		return -1;
#endif
 ...
 */
struct usb_device *usb_alloc_new_device(void *controller,bool attach_to_xhci )
{
	int i;
	debug("New Device %d\n", dev_index);
	...
	usb_dev[dev_index].attach_to_xhci = attach_to_xhci;
	dev_index++;
...
{
	if(udev->attach_to_xhci)
	{
#if defined(CONFIG_USB_XHCI)
		return xhci_usb_alloc_device(udev);
#else
		return -1;
#endif
	}
