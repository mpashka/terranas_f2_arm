...
#ifdef CONFIG_USB2_DPHY_PATCH
		{
			int retry = CONFIG_USB2_DPHY_PATCH_RETRY_COUNTS;
			int d = 0;
			uint32_t soft_reset1;
			uint32_t clock_enable1;
			bool found;
...
again:
			...
			// 0x18000000,0xfffffeab #Release all reset, except USB MAC / port1 PHY
			soft_reset1 = readl(0x18000000);
			writel((soft_reset1 & 0xfffffeab), 0x18000000);
			udelay(100);

			// 0x1800000c,0xffffffef #Enable all clock, except USB
			clock_enable1 = readl(0x1800000c);
			writel((clock_enable1 & 0xffffffef), 0x1800000c);
			udelay(102400);

			// 0x1800000c,0xffffffff #Enable all clock,
			writel(clock_enable1, 0x1800000c);
			udelay(100);
			...
			// 0x18000000,0xffffffff #Release all reset
			writel(soft_reset1, 0x18000000);
			udelay(100);

			// 0x1800000c,0xffffffff #Turn on all clock
			writel(clock_enable1, 0x1800000c);
			udelay(100);
			...
			for (d = 0; d < USB_MAX_DEVICE; d++) {
				...
				if (dev == NULL)
					break;
				if ((dev->descriptor.idVendor == 0x0bda) && (dev->descriptor.idProduct == 0x5411)) {
					found = true;
					break;
				}
			}
			...
			if (!found && retry) {
				printf ("No RTK hub be found, go to CRT reset!\n");
				run_command_list("usb stop", -1, 0);
				goto again;
			}
