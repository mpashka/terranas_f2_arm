...
{
#if !defined(NAS_ENABLE) || !defined(CONFIG_SYS_RTK_NAND_FLASH)
    ...
    if (BOOT_NOR_SERIAL == boot_flash_type) {
        strcpy(tmp_cmdline, "root=/dev/sda1 rootfstype=squashfs rootwait");
    }
#endif
...
#endif
#ifdef CONFIG_CMD_RTKSPI
extern void rtkspi_read32_progress( unsigned int target_address, unsigned int source_address, unsigned int byte_length);
#endif
#ifdef CONFIG_BOOT_FROM_SPI
...
#endif
{
	...
	for(i = 0; i < fw_count; i++) {
		this_entry = (fw_desc_entry_v1_t *)(fw_entry + sizeof(fw_desc_entry_v1_t) * i);
		switch(this_entry->type) {
			case FW_TYPE_KERNEL:
				printf("Kernel:\n");
				memset(str, 0, sizeof(str));
				sprintf(str, "%x", this_entry->target_addr);
				setenv("kernel_loadaddr", str);
				break;
			...
			case FW_TYPE_RESCUE_ROOTFS:
				printf("Rescure RootFS:\n");
				printf("      target addr: 0x%08x\n", this_entry->target_addr);
				if( boot_mode == BOOT_RESCUE_MODE ) {
					memset(str, 0, sizeof(str));
					sprintf(str, "0x%x", this_entry->target_addr);
					setenv("rootfs_loadaddr", str);
					break;
				}
				else {
					printf("      ignore this\n");
			...
			default:
				printf("Pass image type %d\n", this_entry->type);
				continue;
		}
		printf("\t FW Image to 0x%08x, size=0x%08x (0x%08x)\n",
			this_entry->target_addr, this_entry->length, this_entry->target_addr + this_entry->length);
		printf("\t FW Image fr 0x%08x \n", (0x00100000 +this_entry->offset));
		//printf("      fw checksum: 0x%08x\n", this_entry->checksum);

		/* Handle memory layout on compression */
		mem_layout.bIsEncrypted = 0;
		mem_layout.bIsCompressed = this_entry->lzma;
		mem_layout.image_target_addr = this_entry->target_addr;
		get_mem_layout_when_read_image(&mem_layout);
		imageSize = this_entry->length;
		...
		rtkspi_read32_progress( mem_layout.flash_to_ram_addr, (SPI_RBUS_BASE_ADDR + this_entry->offset), imageSize );
...
#ifdef NAS_ENABLE
		if (FW_TYPE_RESCUE_ROOTFS == this_entry->type)
		{
			initrd_size = (this_entry->lzma)?decompressedSize:this_entry->length;
		}
#endif
	}
#else // CONFIG_FW_TABLE_IN_SPI
#ifdef CONFIG_DTB_IN_SPI_NOR
	unsigned int ret;
	// load DTS	
	if (boot_mode == BOOT_RESCUE_MODE || boot_mode == BOOT_ANDROID_MODE)
	{
		ret = rtkspi_load_DT_block(1, CONFIG_FDT_LOADADDR); // rescure
		if( ret ) {
			printf("Rescue DT: fdt addr = 0x%08x, len = 0x%08x\n", CONFIG_FDT_LOADADDR, ret);
		}
		else {
	...
	{
		ret = rtkspi_load_DT_block(0, CONFIG_FDT_LOADADDR); // normal
		if( ret ) {
			printf("Normal DT: fdt addr = 0x%08x, len = 0x%08x\n", CONFIG_FDT_LOADADDR, ret);
		}
		else {
			printf("Normal DT: not found\n");
			return RTK_PLAT_ERR_PARSE_FW_DESC;
		}
	}
	else
...
#endif // end of #ifdef CONFIG_DTB_IN_SPI_NOR
#endif // end of #ifdef CONFIG_FW_TABLE_IN_SPI
...
int rtk_plat_prepare_fw_image_from_SPI(void)
{
	...
	do {
		...
		fw_desc_table_base = FIRMWARE_DESCRIPTION_TABLE_ADDR; // 0x0640_0000
		rtkspi_read32(fw_desc_table_base, _iSPI_base_addr, 512);
		memcpy(&fw_desc_table_v1, (void*)fw_desc_table_base, sizeof(fw_desc_table_v1_t));
		...
		{
			printf("[ERR] %s:Signature(%s) error!\n", __FUNCTION__, fw_desc_table_v1.signature);
			rtk_hexdump("fw table : ", (void*)fw_desc_table_base, 512);
			ret = RTK_PLAT_ERR_PARSE_FW_DESC;
			break;
		}
		//printf("****** %s %d, fw_desc_table_v1.checksum      0x%08x\n", __FILE__, __LINE__, fw_desc_table_v1.checksum);
		//printf("****** %s %d, fw_desc_table_v1.paddings      0x%08x\n", __FILE__, __LINE__, fw_desc_table_v1.paddings);
		//printf("****** %s %d, fw_desc_table_v1.part_list_len 0x%08x\n", __FILE__, __LINE__, fw_desc_table_v1.part_list_len);
		//printf("****** %s %d, fw_desc_table_v1.fw_list_len   0x%08x\n", __FILE__, __LINE__, fw_desc_table_v1.fw_list_len);

		/* copy Firmware Description/Partition/Fw_entry Tables from flash */
		rtkspi_read32(fw_desc_table_base, _iSPI_base_addr, fw_desc_table_v1.paddings);
		...
		printf("****** %s %d, caculated checksum             0x%08x\n", __FILE__, __LINE__, checksum);

		if( fw_desc_table_v1.checksum != checksum )
		{
			printf("[ERR] %s:Checksum not match(0x%x != 0x%x)\n", __FUNCTION__, fw_desc_table_v1.checksum, checksum);
			rtk_hexdump("fw table : ", (void*)fw_desc_table_base, 512);
			ret = RTK_PLAT_ERR_PARSE_FW_DESC;
			break;
		}
		...
		fw_entry_num = fw_desc_table_v1.fw_list_len / sizeof(fw_desc_entry_v1_t);
		if( fw_entry_num < 4 ) {
			printf("[ERR] %s: fw entry descriptors not enought\n", __FUNCTION__, fw_entry_num);
			rtk_hexdump("fw table : ", (void*)fw_desc_table_base, 512);
			ret = RTK_PLAT_ERR_PARSE_FW_DESC;
			break;
		}
		printf("[INFO] fw_entry_num: %d\n", fw_entry_num);

		part_entry = (part_desc_entry_v1_t *)(fw_desc_table_base + sizeof(fw_desc_table_v1_t));
		fw_entry = (fw_desc_entry_v1_t *)(fw_desc_table_base + sizeof(fw_desc_table_v1_t) + fw_desc_table_v1.part_list_len);

		// Parse firmware entries and compute fw_total_len
		switch (fw_desc_table_v1.version)
		{
			case FW_DESC_TABLE_V1_T_VERSION_1:
				printf("[INFO] %s:version is %d; Support FW_DESC_TABLE_V1_T_VERSION_1\n", __FUNCTION__, fw_desc_table_v1.version);
				ret = RTK_PLAT_ERR_OK;
				break;

			case FW_DESC_TABLE_V1_T_VERSION_11:
				printf("[ERR] %s:version is %d; Not support FW_DESC_TABLE_V1_T_VERSION_11\n", __FUNCTION__, fw_desc_table_v1.version);
				ret = RTK_PLAT_ERR_PARSE_FW_DESC;
				break;

			case FW_DESC_TABLE_V1_T_VERSION_21:
				printf("[ERR] %s:version is %d; Not support FW_DESC_TABLE_V1_T_VERSION_21\n", __FUNCTION__, fw_desc_table_v1.version);
				ret = RTK_PLAT_ERR_PARSE_FW_DESC;
				break;
