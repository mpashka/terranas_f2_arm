...
static int do_usb(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
...
#ifdef EXT_USB3_PWR_CTL
		volatile int regAddr = 0x18000368;
		unsigned int regValue;
		regValue = rtd_inl(regAddr);

		if(40 == CONFIG_USB_PWR_GPIO_NUM){
			/* Set MUX on GPIO40: 10 on bit[17,16] */
			regValue |= (0x01 << 17);
			regValue &= (~(0x01 << 16));
		} else if(42 == CONFIG_USB_PWR_GPIO_NUM){
			/* Set MUX on GPIO42: 10 on bit[21,20] */
			regValue |= (0x01 << 21);
			regValue &= (~(0x01 << 20));
		}
		if(40 == CONFIG_USB_PWR_GPIO_NUM || 42 == CONFIG_USB_PWR_GPIO_NUM){
			rtd_outl(regAddr, regValue);
			/* Low active */
			setGPIO(CONFIG_USB_PWR_GPIO_NUM, 0);
		}
#endif
#ifdef CONFIG_USB_UPDATE_WHEN_AC_ON
	...
	if (strncmp(argv[1], "uuid", 4) == 0) {
		if (argc == 4) {
			...
			/* Support MBR & EFI partition only */
			if (stor_dev->part_type != PART_TYPE_DOS &&
			    stor_dev->part_type != PART_TYPE_EFI)
				return 1;
			...
			if (stor_dev->part_type == PART_TYPE_EFI &&
			    get_partition_info_efi(stor_dev,part,&info) != 0)
				return 1;
#endif
