...
#define XHCI_USB_MAX_XFER_BLK	20
...
static int usb_stor_CBI_get_status(ccb *srb, struct us_data *us)
{
	...
	if(us->pusb_dev->attach_to_xhci)
	{
#if defined(CONFIG_USB_XHCI)
		xhci_submit_int_msg(us->pusb_dev, us->irqpipe,
			    ...
			    lbaint_t blkcnt, void *buffer)
{
	...
	maxblksize = dev->attach_to_xhci ? (XHCI_USB_MAX_XFER_BLK) : (USB_MAX_XFER_BLK);
	...
	do {
		...
		if (blks > maxblksize)
			smallblks = maxblksize;
		else
			smallblks = (unsigned short) blks;
retry_it:
		if (smallblks == maxblksize)
			usb_show_progress();
